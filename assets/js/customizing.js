...
$(document).ready( function() { 	// Wait until DOM be ready to start.
    ...
    function timer( time ) {
        var x = setInterval( () => {
            time -= 1;  
            $("#timer").text("Preparation time: " + time + "s ");
            if (time <= 0) {
                $("#timer").text("");
                clearInterval(x);
            }
    ...
    sctructure.forEach( ( value ) =>  {
        ...
        setTimeout( () => {
            ...
            $( "#prgss-01 .progress-bar" ).css({animationName:"barr", width:"100%", animationDuration: value.qBarrMSGDuration + "s"});
